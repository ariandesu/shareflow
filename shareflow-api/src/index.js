...
app.post("/api/file", apiKeyAuth, logUsage, async (c) => {
    ...
    for (const file of files) {
        if (file.size > 10 * 1024 * 1024)
            return c.json({ error: "Each file must be smaller than 10MB" }, 400);
    }
    let code = "";
    for (let i = 0; i < 5; i++) {
        code = generateCode(4);
...
app.post("/api/file/p2p", apiKeyAuth, logUsage, async (c) => {
    ...
    const size = body.size !== undefined ? body.size : p2pFiles.reduce((sum, f) => sum + (f.size || 0), 0);
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (!exists && !file && !batch)
            break;
    }
    const sessionFiles = p2pFiles.length > 0
        ? p2pFiles.map((f) => ({ name: f.name, size: f.size, mimeType: f.mimeType || 'application/octet-stream' }))
...
app.get("/api/file/:code", apiKeyAuth, logUsage, async (c) => {
    ...
    if (batchObj) {
        ...
        return c.json({
            ...
            size: batch.files.reduce((sum, f) => sum + f.size, 0),
    ...
    if (p2pObj) {
        ...
        return c.json({ type: 'p2p', name: data.name || (data.files[0] && data.files[0].name), size: data.size, mimeType: data.mimeType, createdAt: data.createdAt, offer: data.offer, files: data.files || [{ name: data.name, size: data.size, mimeType: data.mimeType }], count: (data.files || []).length });
    }
...
app.get("/api/file/:code/download/:index?", async (c) => {
    ...
    if (object) {
        ...
        const expiresAt = Number(fileObj.customMetadata?.expiresAt);
        if (expiresAt && Date.now() > expiresAt) {
            c.executionCtx.waitUntil(c.env.BUCKET.delete(`batch-${code}-${index}`));
            return c.json({ error: "File has expired" }, 410);
        }
        const headers = new Headers();
        fileObj.writeHttpMetadata(headers);
        headers.set('etag', fileObj.httpEtag);
        if (fileObj.customMetadata?.filename)
            headers.set('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(fileObj.customMetadata.filename)}`);
    ...
    if (single.customMetadata?.filename)
        headers.set('Content-Disposition', `attachment; filename*=UTF-8''${encodeURIComponent(single.customMetadata.filename)}`);
    return new Response(single.body, { headers });
});
export default app;
